...
class BaseService(Generic[ModelType]):
    ...
    async def get_list(self, limit: Optional[int] = None):
        async with self.db_session as session:
            ...
            query = await session.execute(select(self.table).limit(limit).order_by(-self.table.id.desc()))
            return query.scalars().all()

    async def get_one(self, id):
        async with self.db_session as session:
            db_item = await session.execute(select(self.table).filter(self.table.id == id))
    ...
    async def update(self, data):
        async with self.db_session as session:
            await session.execute(update(self.table), [data.dict()])
            await session.commit()
